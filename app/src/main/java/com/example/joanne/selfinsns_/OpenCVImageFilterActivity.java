...
public class OpenCVImageFilterActivity extends AppCompatActivity {
    ...
    private void showDialogforPermission(String msg) {
        ...
        final AlertDialog.Builder myDialog = new AlertDialog.Builder(  OpenCVImageFilterActivity.this);
    ...
    private void imageprocess_and_showResult() {
        ...
        Log.e("TAG", String.valueOf(bitmapInput));
        Utils.matToBitmap(img_input, bitmapInput); //유틸 클래스를 이용하여 bmp클래스로 변환.
        Log.e("TAG", String.valueOf(bitmapInput));
        Log.e("TAG", String.valueOf(img_input));
        imageVIewInput.setImageBitmap(bitmapInput); //필터링전사진

        Bitmap bitmapOutput = Bitmap.createBitmap(img_output.cols(), img_output.rows(), Bitmap.Config.ARGB_8888);
        Log.e("TAG", String.valueOf(bitmapOutput));
        Utils.matToBitmap(img_output, bitmapOutput);
        imageVIewOuput.setImageBitmap(bitmapOutput); //필터링 후 사진
    }

    private void read_image_file() {
        copyFile("profile01.jpg");

        img_input = new Mat();
        img_output = new Mat();

        loadImage("profile01.jpg", img_input.getNativeObjAddr());
    }
